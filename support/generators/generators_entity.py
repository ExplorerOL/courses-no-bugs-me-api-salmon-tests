...
class GeneratorsEntity(Generic[T]):
    ...
    def generate_entity_with_random_data(entity_type: T) -> T:
        ...
        if not is_dataclass(entity_type):
            raise ValueError(
                f'Генерация атрибутов возможна только для датакласса! Переданный тип: {entity_type!r}!'
            )
    ...
    @staticmethod
    def generate_entities_with_random_data(type: T, posts_count: int) -> list[Any]:
        return [GeneratorsEntity.generate_entity_with_random_data(entity_type=T) for _ in range(posts_count)]
