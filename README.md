
# Финальное задание курса "Архитектура API-тестов"
Написаны API-тесты на Python с использованием ООП, паттернов проектирования и лучших практик

## Запуск тестов на Windows с использованием uv
### Установка uv глобально
```
python -m pip install uv
```
Добавить в переменную окружения Path путь к uv. При глобальной установке uv будет находиться в директории
C:\Users\<имя_пользователя>\AppData\Local\Programs\Python\Python311\Scripts


### Склонировать репозиторий
```
git clone https://github.com/ExplorerOL/courses-no-bugs-me-salmon-tests.git
```

### Запуск тестов
```
cd <путь к директории проекта>
uv run pytest
```
При запуске тестов автоматически создастся виртуальное окружение и будут установлены зависимости

### Генерация отчета
Для генерации отчета нужно установить Java, скачать утилиту для генерации отчетов allure, прописать путь до диретории bin
утилиты в Path, а затем выполнить команду:
```
allure generate --single-file .\reports\allure-results\ -o .\reports\allure-report --clean
```

### Форматирование и линтинг кода тестов
Форматирование и линтинг кода выполняется с помощью ruff и pre-commit-hook перед выполнением коммита git.